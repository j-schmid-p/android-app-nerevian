...
//    private val BASE_URL = "http://127.0.0.1:8000/api"
    ...
    //private val BASE_URL = "http://10.90.132.211:8000/api"

    fun login(email: String, password: String): JSONObject? {
        return try {
            val connection = openConnection("$baseUrl/auth/login", "POST", null)
    ...
    fun getMe(token: String): JSONObject? {
        return try {
            val connection = openConnection("$baseUrl/auth/me", "GET", token)
            readJSONObject(connection)
        } catch (e: Exception) {
    ...
    fun updateProfile(
        ...
        return try {
            val connection = openConnection("$baseUrl/profile", "PUT", token)
            val body = JSONObject().apply {
    ...
    fun getTrackingOptions(token: String, offerId: Int): JSONObject? {
        return try {
            val connection = openConnection("$baseUrl/offers/$offerId/tracking", "GET", token)
            readJSONObject(connection)
        } catch (e: Exception) {
    ...
    fun getCurrentTracking(token: String, offerId: Int): JSONObject? {
        return try {
            val connection = openConnection("$baseUrl/offers/$offerId/tracking/current", "GET", token)
            readJSONObject(connection)
        } catch (e: Exception) {
    ...
    fun getOffersList(token: String): List<Offer>? {
        return try {
            val connection = openConnection("$baseUrl/offers", "GET", token)
    ...
    fun logout(token: String): JSONObject? {
        return try {
            val connection = openConnection("$baseUrl/auth/logout", "POST", token)
            readJSONObject(connection)
        } catch (e: Exception) {
